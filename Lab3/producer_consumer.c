...
void *producer(void *parg)
{
  ...
  while (1)
  {
    ...
    pthread_mutex_lock(fifo->mutex); // For synchronization purposes between consumers and producers we add a mutex lock
    /*
    ...
    {
      printf("prod %d:  FULL.\n", my_tid);
      pthread_cond_wait(fifo->notFull, fifo->mutex); // For full buffer, we need to wait for a signal from the consumers
    }
    ...
    pthread_cond_signal(fifo->notEmpty); // Signal the consumer each time an item gets produced
    pthread_mutex_unlock(fifo->mutex);
  }

  printf("prod %d:  exited\n", my_tid);
  pthread_cond_signal(fifo->notEmpty); // Producers should signal consumers upon terminating
  pthread_mutex_unlock(fifo->mutex);
  return (NULL);
}
...
void *consumer(void *carg)
{
  ...
  while (1)
  {
    ...
    {
      printf("con %d:   EMPTY.\n", my_tid);
      pthread_cond_wait(fifo->notEmpty, fifo->mutex); // For empty buffer, we need to wait for a signal from the consumers
    }
  ...
  printf("con %d:   exited\n", my_tid);
  pthread_cond_signal(fifo->notEmpty); // Consumer sends signal to every producer to make sure they terminate
  pthread_mutex_unlock(fifo->mutex);
  return (NULL);
}
...
int main(int argc, char *argv[])
{
  ...
  procount = (int *)calloc(0, sizeof(int));
  if (procount == NULL)
  {
  ...
  concount = (int *)calloc(0, sizeof(int));
  if (concount == NULL)
  {
